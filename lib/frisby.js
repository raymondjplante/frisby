...
//HTTP status in provided status codes expect helper -- works with arrays or single values
Frisby.prototype.expectStatusIn = function(statusCodes) {
  var self = this;

  //Convert array to string for regex
  if(Array.isArray(statusCodes)){
    statusCodes = statusCodes.join("|");
  }

  this.current.expects.push(function() {
    expect(self.current.response.status).toMatch(new RegExp("^" + statusCodes + "$"));
  })
  return this;
};
